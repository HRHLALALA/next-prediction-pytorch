...
def read_data(args, data_type):
  ...
  if args.preload_features:
    ...
    person_feat_dim = args.person_feat_dim
    T_in = args.obs_len
    split = data_type

    obs_person_features = np.zeros(
      (num_examples, T_in, person_h, person_w, person_feat_dim), dtype='float32')

    def read_file(person_feat_path, split, boxid, i, j):
      featfile = os.path.join(
        person_feat_path, split, '%s.npy' % boxid)

      obs_person_features[i, j] = np.squeeze(
        np.load(featfile), axis=0)

    threads = []
    for i in trange(len(data['obs_boxid'])):
    ...
    data['obs_person_features'] = obs_person_features
  #################### To memory ###############################33
...
def evaluate(dataset, config, tester):
  ...
  for  evalbatch in tqdm.tqdm(dataset.get_batches(config.batch_size, \
    ...
    pred_out, future_act, grid_pred_1, grid_pred_2, \
        traj_class_logits, _ = tester.step(evalbatch)
...
class Saver:
  ...
  def restore(self,engine, loadpath):
    ...
    if isinstance(engine, Trainer):
      engine.config = ckpt['args']
      engine.opt.load_state_dict(ckpt['optim_state_dict'])
