...
def prepro_each(traj_path, split, prepro_path, args):
  ...
  pred_traj = seq_list[:, -pred_len:, :]

  obs_traj_rel = seq_list_rel[:, :obs_len, :]
  pred_traj_rel = seq_list_rel[:, -pred_len:, :]
  ...
  if args.add_kp:
    ...
    pred_kp = kp_list[:, -pred_len:, :, :]  # for visualization
  ...
  if args.add_grid:
    ...
    pred_seq_grid_class = seq_grid_class_list[:, :, -pred_len:]
    pred_seq_grid_target = seq_grid_target_list[:, :,  -pred_len:]
    data.update({
  ...
  if args.add_scene:
    ...
    pred_scene = scene_list[:, -pred_len:, :]
